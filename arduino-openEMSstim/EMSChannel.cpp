...
EMSChannel::EMSChannel(uint8_t channel_to_Pads, uint8_t channel_to_Pads_2,
                       uint8_t led_active_pin, AD5252* digitalPoti, uint8_t wiperIndex, uint8_t multiplex_pin, bool multiplex_false = false) {
  intensity = 0;
  activated = false;
  multiplexed = false;
  ...
  this->multiplex_pin = multiplex_pin;
  this->multiplex_false = multiplex_false;
...
void EMSChannel::multiplex() {
  multiplexed = true;
  pinMode(multiplex_pin, OUTPUT);
  digitalWrite(multiplex_pin, multiplex_false);
}
