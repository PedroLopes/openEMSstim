...
1. A bluetooth 4.0 device for controlling the openEMSstim, a simple smartphone.
2. cable from your EMS/TENS device to the openEMSstim
3. a off the shelve muscle stimulator (for instance a TENS / massage device you bought legally in your country/doctor/etc)
4. cables to connect the openEMSstim to the electrodes (which attacht to your skin and deliver impulses)
5. the openEMSstim control board (the open source device that Max and Tim designed, and I gently modded)
6. pre-gelled EMS/TENS electodes 

All the following steps are accomplished in our <a href="http://plopes.org/ems/#openEMSstimTutorial">video tutorial (complete tutorial)</a> as well. I recommend you to first read them through, familiarize yourself with the gear and watch the video later. Pause the video when needed and execute each step carefully, always checking the hardware as recommended and not skipping any step. 
...
The video for this section is this <a href="http://plopes.org/ems/#poweringUp">here (powering up the openEMSstim)</a>. 
...
The video for this section is <a href="http://plopes.org/ems/#androidApp">here (connecting to the Android App)</a>. 
...
3. Check whether the bluetooth can see your EMS device. We suggest you use [BLEscanner](https://play.google.com/store/apps/details?id=com.macdom.ble.blescanner) app (free). It will show you the nearby Bluetooth LE devices and you can write down the name of the openEMSstim, it is probably something like openEMSstim01 (see here for my naming conventions). ![Scan the BT devices to get the openEMSstim name](../extra/images/android-app/1.FindTheNameOfDevice.jpeg)
4. Open one of our EMS apps, I recommend: test-openEMSstim.apk (install it just by tapping on it and Android will install it, you can even copy it via USB by downloading it <a href="http://plopes.org/ems/#getAndroidApp">here</a>)
5. Inside the "test-openEMSstim" app:
	1. tap on the top right corner, on the "more" button ![Tap the top right corner for more options](../extra/images/android-app/2.CheckIfNameIsCorrectOnApp.jpeg)
	2. Select "Current Device".
	3. Using the on-screen keyboard type the device name you saw on the bluetooth scanner.
![Change the name](../extra/images/android-app/3.ChangeTheName.jpeg)
	4. Now tap "connect".
![Select Connect](../extra/images/android-app/4.Connect.jpeg)
	5. The openEMSstim app turns green if it connected successfully. If not, simply reboot the app and try again (sometimes happens). Also, you can open your openEMSstim board and see if the blue LED inside is on (if bluetooth connection is estabelished, the blue LED goes on).
![Is it green then it worked](../extra/images/android-app/5.IfGreen_IsConnected_Else_RebootApp.jpeg)
6. The app and openEMSstim communicate properly. You can now test the channels independently:
	1. Tap and hold in the "left channel" button on the app. You should see a "green" LED on the openEMSstim when you do that.  
![Test left channel](../extra/images/android-app/6.TestLEDsForLeft.jpeg)	2. Tap and hold in the "right channel" button on the app. You should see a "red" LED on the openEMSstim when you do that.  
![Test right channel](../extra/images/android-app/7.TestLEDsForRight.jpeg)	3. All is fine. 	 
...
The video for this section is <a href="http://plopes.org/ems/#electrodePlacement">here (placing two electrodes on the Palm Flexor)</a>. 
...
The video for this section is <a href="http://plopes.org/ems/#makingConnections">here (connecting electrodes and EMS to the openEMSstim)</a>. 

This is simple but make sure you plug into everything into its own socket, not the other way around:
1. first plug the EMS machine to the rigth most socket (**side of the USB connection**), like this:
...
This is an abstract diagram to help you read the connections. The real thing might as well look like this:

![How to connect your board](../extra/images/getting-started/5-actuated-hand.jpeg)
...
The video for this section is <a href="http://plopes.org/ems/#calibrating">here (calibrating, step by step)</a>. . 

1. Start your EMS machine at minimum intensity. Open one of the channels. 
![How to connect your board](../extra/images/calibrating/1.ChannelIsOff.jpeg)
...
![How to connect your board](../extra/images/calibrating/2.OpenSlowly.jpeg)
